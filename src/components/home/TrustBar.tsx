  ...
  return (
    <section ref={containerRef} className="py-12 bg-black border-y border-white/10 overflow-hidden relative z-10">
      <div className="container mx-auto px-6 mb-8 text-center">
        <p className="text-sm font-medium text-muted-foreground uppercase tracking-widest">Our Impact</p>
      </div>
          ...
          {stats.map((stat, index) => (
            <div key={index} className="flex items-center justify-center mx-12">
              <div className="text-center">
                ...
                <p className="text-xs md:text-sm font-mono uppercase tracking-widest text-muted-foreground/80">{stat.label}</p>
              </div>
            </div>
          ))}
        </div>
